...
import { createArticleSubmissionsQueryOptions } from "./query_options/articleSubmissionsQueryOptions";
import { useState } from "react";
import './Submissions.css'
import { useNavigate } from "react-router-dom";
import { processSubmission, rejectSubmission } from "../api/articleAPI";
    ...
    <div className="submission-card">
      ...
      <div className="submission-footer">
        ...
        <div className="submission-actions">
          ...
          >
            {(rejectState == 0) ? `Rejeitar` : `Confirmar`}
          </button>
        </div>
      </div>
    </div>
...
function Submissions(){
    ...
    async function handleApprove(id: number){
        ...
        queryClient.invalidateQueries({queryKey:["article_submissions", page, pageSize]})
    ...
    return <div className="submissions-container">
        ...
        <div className="submissions-pagebox">
            {(submissionsQuery.data?.length ?? 0) > 0 ?(submissionsQuery.data?.map(s => <SubmissionCard key={s.id} submission={s} onApprove={handleApprove} onReject={handleReject} onPreview={() => navigate("/preview", {state:s}) }/>)) : (<p className="empty-notif">Hmmm... pelo visto não tem nada aqui ainda</p>)}
        </div>
